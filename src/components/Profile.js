...
const Profile = () => {
  ...
  const getUserPost = useStoreActions(
    actions => actions.getUserPost
  );
  const setUserPost = useStoreActions(
    actions => actions.setUserPost
  );
  const toggleEditModeAction = useStoreActions(
    actions => actions.toggleEditModeAction
  );
  ...
  const setUser = (userUpdateDetails) => {
    setUserPost(userUpdateDetails)
  }

  const toggleEditMode = (editMode) => {
    toggleEditModeAction(editMode)
  }
  ...
  const onSubmit = data => {
    if ( data.name !== userDetails.name || data.email !== userDetails.email) {
      setUser(data)
      // console.log("Details changed, saving: " + JSON.stringify(data))
      toggleEditMode(editMode)
    } else {
      // console.log("Details not changed: " + JSON.stringify(data))
      toggleEditMode(editMode)
    }
  };
  ...
  // Holy fuck who knew you could call useEffect twice!?!?!?!
  useEffect (() => {
    getUserPost()
  },[getUserPost]);
  ...
  if (userDetails.isFetching) {
    ...
    const EditButton = () => {
      return (<button className='editButton' onClick={editButton}>Edit</button>)
    }
    
    returnItems = [
      <div key={'returnItemsDiv'} className='title' style={{padding: "3px"}}>
        <div key='profile' className="profile">Player Profile</div>
          <form key={'form'} onSubmit={handleSubmit(onSubmit)}>
          <label key={'label1'} className='label'>Name:</label>
            <input key={'input1'} className='input'  type="text" name="name" {...register('name')} disabled={!editMode}/>
            <ErrorMessage errors={errors} name="name" />
          <label key={'label2'} className='label'>Email:</label>
            <input key={'input2'} className='input' type="email" name="email" {...register('email')} disabled={!editMode}/>
            <ErrorMessage errors={errors} name="email" />
          {editMode ? <input key={'submit1'} className='saveButton' type="submit" value="Save"/> : <EditButton key='edit'/>}
        </form>
      </div>
    ]
